...
namespace MySpace
{
    [AddComponentMenu("Extension/My Scroll Rect", 37)]
    [SelectionBase]
    [ExecuteInEditMode]
    ...
    public class MyScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        {
            get { return (int)Math.Round(m_Content.localPosition.y/CellSize.y); }
            set
            {
                //if (CurrentStep != value)
                {
                    m_Snapping = true;
                    float from = m_Content.localPosition.y;
                    float to = value * CellSize.y;
                    float distance = Math.Abs(from - to);
                    float time = 0.15f * distance / 90f;
                    if (time > 0.5f) time = 0.5f;
                    DOTween.To((f) =>
                    {
                        m_Content.localPosition = new Vector3(m_Content.localPosition.x, f, m_Content.localPosition.z);
        ...
        void RunSnap(bool noTime = false)
        {
            ...
            float pos = m_Content.localPosition.y;
            int v1 = (int) Math.Floor(pos/CellSize.y);
            int v2 = (int) Math.Ceiling(pos/CellSize.y);
            
            float snap;
            if (Math.Abs(v1 * CellSize.y - pos) < Math.Abs(v2 * CellSize.y - pos))
            {
                snap = v1*CellSize.y;
            }
            else
            ...
            if (noTime)
            {
                m_Content.localPosition = new Vector3(m_Content.localPosition.z, snap, m_Content.localPosition.z);
                m_Snapping = false;
        ...
        void TweenMoveY(float from, float to, float time)
        {
            ...
            m_SnapTweener = DOTween.To((f) =>
            {
                ...
                {
                    m_Content.localPosition = new Vector3(m_Content.localPosition.z, f, m_Content.localPosition.z);
                }
                else
                {
                    m_Snapping = false;
