...
namespace MySpace
{
    [AddComponentMenu("Extension/My Scroll Rect Horizontal", 37)]
    [SelectionBase]
    [ExecuteInEditMode]
    [DisallowMultipleComponent]
    [RequireComponent(typeof(RectTransform))]
    public class MyScrollRectHorizontal : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        protected MyScrollRectHorizontal()
        {
            flexibleWidth = -1;
        }
        ...
        #region RunSnap Child

        public Action OnSnapFinish = null; 
        public bool Snap;
        public int CurrentStep
        {
            get { return (int)Math.Round(m_Content.localPosition.x/CellSize.x); }
            set
            {
                //if (CurrentStep != value)
                {
                    m_Snapping = true;
                    float from = m_Content.localPosition.x;
                    float to = value * CellSize.x;
                    float distance = Math.Abs(from - to);
                    float time = 0.15f * distance / 90f;
                    if (time > 0.5f) time = 0.5f;
                    DOTween.To((f) =>
                    {
                        m_Content.localPosition = new Vector3(f, m_Content.localPosition.y, m_Content.localPosition.z);
                    }, from, to, time).SetEase(Ease.OutQuad).OnComplete(() =>
                    {
                        m_Snapping = false;
                    });
                }
        ...
        public float MinVelocityToSnap = 150;
        private bool m_Snapping = false;
        private Tweener m_SnapTweener = null;

        void RunSnap(bool noTime = false)
        {
            m_Snapping = true;

            float pos = m_Content.localPosition.x;
            int v1 = (int) Math.Floor(pos/CellSize.x);
            int v2 = (int) Math.Ceiling(pos/CellSize.x);
            
            float snap;
            if (Math.Abs(v1 * CellSize.x - pos) < Math.Abs(v2 * CellSize.x - pos))
            {
                snap = v1*CellSize.x;
            }
            else
            ...
            if (noTime)
            {
                m_Content.localPosition = new Vector3(snap, m_Content.localPosition.z, m_Content.localPosition.z);
                m_Snapping = false;
                if (OnSnapFinish != null)
                    OnSnapFinish();
            }
            else
            {
                TweenMoveY(pos, snap, SnapTime);
            }
        }

        void TweenMoveY(float from, float to, float time)
        {
            if (m_SnapTweener != null)
                m_SnapTweener.Kill();

            m_SnapTweener = DOTween.To((f) =>
            {
                if (m_Dragging == false && m_Snapping == true)
                {
                    m_Content.localPosition = new Vector3(f, m_Content.localPosition.z, m_Content.localPosition.z);
                }
                else
                {
                    m_Snapping = false;
            ...
            }, from, to, time).SetEase(Ease.OutBack).OnComplete(() =>
            {
                m_Snapping = false;
                if (m_SnapTweener != null)
                    m_SnapTweener.Kill();
        ...
        protected virtual void LateUpdate()
        {
            ...
            if (!m_Dragging && (offset != Vector2.zero || m_Velocity != Vector2.zero))
            {
                ...
                {
                    bool isReachClamp = false;
                    if (m_MovementType == MovementType.Clamped)
                    {
                        offset = CalculateOffset(position - m_Content.anchoredPosition);
                        position += offset;

                        if (Math.Abs(offset.magnitude) > 0.05f)
                        {
                            isReachClamp = true;
                    ...
                    if (Snap && m_Dragging == false && m_Snapping == false && isReachClamp == false)
                    {
                        if (m_Velocity.magnitude < MinVelocityToSnap)
                        {
                            m_Velocity = Vector2.zero;
                            RunSnap();
                        }
                    ...
                    if (Snap == false || m_Snapping == false)
                    {
                        SetContentAnchoredPosition(position); //update   
                    } 
                }
            }
